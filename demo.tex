...
\and
Adam Wagenhäuser \\ \href{mailto:adam.wagenhaeuser@fau.de}{\texttt{adam.wagenhaeuser@fau.de}}}
...
%\texttt{rust-defs} -- for definition like listings. Colors Traits, Structs, and Enums differently.
...
%\texttt{rust} -- for normal Rust code. Less coloring.
...
%\texttt{rust-bw} -- the minimal style. No colors, keywords are bold.

\begin{lstlisting}[style=lined,style=rust-bw]
impl Bar for crate::Foo {
    // The perfect implementation
    #[must_use]
    fn bar(n: i32) -> Option<File> {
        let a = 42;
        if let Some(n) = Foo::foo(a.pow(12)) {
            return "Got some";
        }
    }
}
